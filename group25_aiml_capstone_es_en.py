# -*- coding: utf-8 -*-
"""Group25_AIML_Capstone_ES_EN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1j68MtYl3EpD368nDuuUKAm3jZScdDvhS
...
# Uploading the excel file with Spanish translations generated using Azure Translate API
...
train_es = train_df['Spanish Sentence'].tolist()
test_es = test_df['Spanish Sentence'].tolist()

print(f'Length of Spanish Train set: {len(train_es)}')
print(f'Length of Spanish Test set: {len(test_es)}')
...
"""# **Translation from Spanish to English using facebook/mbart model**"""

# Use a pipeline as a high-level helper
from transformers import pipeline

# Create a pipeline for translation from Spanish to English
pipe_2 = pipeline("translation", model="facebook/mbart-large-50-many-to-many-mmt", src_lang="es_XX", tgt_lang="en_XX")

# Generate translations from Spanish to English for test set
translations_es_en = []

translated_sentences = pipe_2(test_es)

for sentence in translated_sentences:
  translations_es_en.append(sentence['translation_text'])

translations_es_en[0:5]
...
for i in range(len(test_en)):
  bleu_scores_es_en.append((sentence_bleu([test_en[i].split()], translations_es_en[i].split())))
  if(i % 300 == 0):
    print(i, 'BLEU score -> {}'.format(sentence_bleu([test_en[i].split()], translations_es_en[i].split())))

# Store translations in an excel file
data = {
    'Spanish Sentence': test_es,
    'Translated Sentence': translations_es_en,
    'Original English Sentence': test_en,
    'BLEU Score': bleu_scores_es_en,
    'Role': test_role_en,
    'Labeled Gender': test_label_en
}
df = pd.DataFrame(data)

df.to_excel('translations_es_en_1.xlsx', index=False)
...
def evaluate(data, golds):
    ...
    print()
    print("No of predicted neutral sentences : ", pred_neul)
    print("percentage neutral",pred_neul/len(data)*100 )
    print()
    print("Out of neutral sentences, no of actually male : ", m)
    print("percentage neutral actually male", m/pred_neul*100)
    print()
    print("Out of neutral sentences, no of actually female : ", f)
    print("percentage neutral actually female", f/pred_neul*100)
    print()
    print("Out of neutral sentences, no of actually neutral : ", neul)
    print("percentage neutral actually neutral", neul/pred_neul*100)

    return labels

# Load the translations
df1 = pd.read_excel('translations_es_en_1.xlsx')
df1.head()

df1.shape

# Filtering records where BLEU score is less than 10^-200 to remove hallucinations
...
filtered_translations_en_es = [test_es[i] for i in df1.index]
filtered_translations_es_en = [translations_es_en[i] for i in df1.index]
...
filtered_bleu_scores_es_en = [bleu_scores_es_en[i] for i in df1.index]
filtered_test_role_en = [test_role_en[i] for i in df1.index]
...
data = {
    'Spanish Sentence': filtered_translations_en_es,
    'Translated Sentence': filtered_translations_es_en,
    'Original English Sentence': filtered_test_en,
    'BLEU Score': filtered_bleu_scores_es_en,
...
df.to_excel('translations_es_en_2.xlsx', index=False)
...
# Load your dataset
file_path = "/content/translations_es_en_2.xlsx"  # Replace with your file path - run once for base model output and fine-tuned model output
...
SPD measures the difference in the expected outcome between the privileged and unprivileged groups. A positive SPD indicates that the model is biased towards the privileged group, while a negative SPD indicates that the model is biased towards the unprivileged group.

The SPD scores range from 0.005 to 0.330, indicating that the model is biased towards the male group.
...
DI measures the ratio of the expected outcome for the privileged group to the expected outcome for the unprivileged group. A DI of 1 indicates that the model is fair, while a DI greater than 1 indicates that the model is biased towards the privileged group.

The DI scores range from 0.0 to inf, indicating that the model is biased towards the male group.

# **Bias Mitigation**

In this section we will mitigate bias using fine tuning. For fine tuning, we require language pair train set where source language is Spanish and target language is English.
"""

# Store train set
data = {
    'Source Spanish Sentence': train_es,
...
df.to_excel('train_set_en_es.xlsx', index=False)
...
df = pd.read_excel("train_set_en_es.xlsx")

es_source_sentences = df['Source Spanish Sentence'].tolist()  # Spanish sentences
...
# Initializing list to store Spanish to English translations from fine-tuned model
ft_translations_es_en = []
...
inputs = fine_tuned_tokenizer(test_es, return_tensors="pt", padding=True, truncation=True)

# Generate the translations (translate from Spanish to English)
...
ft_translations_es_en.extend(translations)

len(ft_translations_es_en)

# Compute BLEU score between translated text and original English text
from nltk.translate.bleu_score import sentence_bleu

ft_bleu_scores_es_en = []

for i in range(len(test_en)):
  ft_bleu_scores_es_en.append((sentence_bleu([test_en[i].split()], ft_translations_es_en[i].split())))

# Store translations in an excel file
data = {
    'Spanish Sentence': test_es,
    'Translated Sentence': ft_translations_es_en,
    'Original English Sentence': test_en,
    'BLEU Score': ft_bleu_scores_es_en,
    'Role': test_role_en,
    'Labeled Gender': test_label_en
}
df = pd.DataFrame(data)

df.to_excel('ft_translations_es_en_1.xlsx', index=False)

df1 = pd.read_excel('ft_translations_es_en_1.xlsx')
df1.head()
...
# Filtering data by removing filtered indexes
filtered_translations_en_es = [test_es[i] for i in df1.index]
filtered_translations_es_en = [ft_translations_es_en[i] for i in df1.index]
filtered_test_en = [test_en[i] for i in df1.index]
filtered_test_label_en = [test_label_en[i] for i in df1.index]
filtered_test_role_en = [test_role_en[i] for i in df1.index]
filtered_bleu_scores_es_en = [ft_bleu_scores_es_en[i] for i in df1.index]
filtered_test_role_en = [test_role_en[i] for i in df1.index]
...
df.to_excel('ft_translations_es_en_2.xlsx', index=False)
